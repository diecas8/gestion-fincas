// productor.js - Lógica completa del productor (Corregida)
...
const basemaps = {
  googleSat: L.tileLayer('https://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', { subdomains: ['mt0','mt1','mt2','mt3'], maxZoom: 20, attribution: 'Google' }),
  esriSat: L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', { maxZoom: 18, attribution: 'Esri' }),
...
async function cargarPropietario() {
  ...
  div.innerHTML = propietario 
    ? `<strong>${propietario.nombre_completo}</strong><br>${propietario.cedula}<br>${propietario.telefono || 'Sin teléfono'}<br>${propietario.correo || currentUser.email}`
    : '<span style="color:#dc2626;">⚠️ Completa tus datos para continuar</span>';
}
...
function initMap() {
  ...
  const maps = [
    { id: 'googleSat', label: '🛰️ Satélite' },
    { id: 'esriSat', label: '🌍 ESRI' },
    { id: 'topo', label: '🏔️ Topo' },
    { id: 'osm', label: '🗺️ OSM' }
  ];
  container.innerHTML = maps.map(m => `<button class="btn-primary btn-outline" data-bm="${m.id}">${m.label}</button>`).join('');
...
async function cargarFincas() {
  const { data, error } = await sb.from('fincas').select('*').eq('user_id', currentUser.id).order('id', { ascending: true });
...
function renderFincasList() {
  ...
  if (!fincas.length) { container.innerHTML = '<small style="color:#94a3b8;">Sin fincas. Crea una.</small>'; return; }
  container.innerHTML = fincas.map(f => `
    <div class="finca-card" data-id="${f.id}">
      ...
      <br><small style="color:#64748b;">${f.ubicacion || 'Sin ubicación'}</small>
    </div>`).join('');

  document.querySelectorAll('.finca-card').forEach(card => {
    card.addEventListener('click', () => seleccionarFinca(fincas.find(f => f.id == card.dataset.id)));
  });
}
...
function renderLotesList() {
  ...
  if (!lotes.length) { container.innerHTML = '<small style="color:#94a3b8;">Sin lotes. Dibuja el primero.</small>'; return; }
  container.innerHTML = lotes.map(l => `
    <div class="lote-card" data-id="${l.id}">
      <strong>${l.nombre}</strong> 
      <span style="background:#e2e8f0; padding:2px 8px; border-radius:20px; font-size:12px; margin-left:4px;">${l.cultivo || 'General'}</span>
  ...
  if (features.length) {
    capaLotes = L.geoJSON({ type: 'FeatureCollection', features }, {
      ...
      onEachFeature: (f, layer) => layer.bindPopup(`
        ...
        Cultivo: ${f.properties.cultivo || 'N/A'}<br>
        Área: ${f.properties.area_ha ? f.properties.area_ha.toFixed(2)+' ha' : 'Calculada'}
      `)
    }).addTo(map);
    map.fitBounds(capaLotes.getBounds(), { padding: [50, 50] });
  }
}
...
function mostrarFormularioFinca(fincaExistente, geom, areaHa, layerTemp) {
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    <textarea id="fincaDesc" class="modal-input" placeholder="Descripción (opcional)">${fincaExistente?.descripcion || ''}</textarea>
  `;
  ...
  const guardar = async () => {
    ...
    if (geom) {
      data.geom = geom.type === 'Polygon' ? { type: 'MultiPolygon', coordinates: [geom.coordinates] } : geom;
    } else {
      toast('Error: No se detectó el polígono');
      return;
    }
    ...
    if (error) { toast('Error: ' + error.message); console.error(error); }
      ...
      toast(isEdit ? 'Finca actualizada' : 'Finca guardada correctamente');
...
// Eventos principales
document.getElementById('editarPropietarioBtn').addEventListener('click', () => {
...
document.getElementById('nuevaFincaBtn').addEventListener('click', () => {
...
document.getElementById('nuevoLoteBtn').addEventListener('click', () => {
  if (!fincaSeleccionada) { toast('Selecciona una finca primero'); return; }
  iniciarDibujo((geom, areaHa, layer) => {
    toast('Módulo de lotes: listo para integrar guardado.');
    // Aquí iría la lógica similar a mostrarFormularioFinca para lotes
  });
});

document.getElementById('exportarMapaBtn')?.addEventListener('click', () => toast('Exportación PDF en desarrollo'));

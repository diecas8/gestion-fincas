// productor.js - versión corregida
...
async function cargarPropietario() {
  ...
  if (error) toast('Error cargando propietario');
  propietario = data;
...
function initMap() {
  ...
  cargarFincas(); // función async, pero no esperamos aquí
}

async function cargarFincas() {
  const { data, error } = await sb.from('fincas').select('*').eq('user_id', currentUser.id).order('id');
  if (error) toast('Error cargando fincas');
  fincas = data || [];
  renderFincasList();
...
async function cargarLotes(fincaId) {
  ...
  if (error) toast('Error cargando lotes');
  lotes = data || [];
  renderLotesList();
}
...
function mostrarFormularioFinca(fincaExistente, geom, areaHa, layerTemp) {
  ...
  const guardar = async () => {
    const nombre = document.getElementById('fincaNombre').value;
    ...
    if (isEdit && fincaExistente) {
      const { error } = await sb.from('fincas').update(data).eq('id', fincaExistente.id);
      if (error) toast('Error: ' + error.message);
      else { toast('Finca actualizada'); if (layerTemp) layerTemp.remove(); }
    } else {
      const { error } = await sb.from('fincas').insert([data]);
      if (error) toast('Error: ' + error.message);
      else { toast('Finca creada'); if (layerTemp) layerTemp.addTo(drawnItems); }
    }
...
function mostrarFormularioLote(loteExistente, geom, areaHa, layerTemp) {
  const isEdit = !!loteExistente;
  const cultivoActual = loteExistente ? loteExistente.cultivo : 'cafe';
  document.getElementById('modalTitle').innerText = isEdit ? 'Editar lote' : 'Nuevo lote';
  document.getElementById('modalBody').innerHTML = `
    <input id="loteNombre" class="modal-input" placeholder="Nombre del lote" value="${loteExistente?.nombre || ''}">
    <select id="cultivoSelect" class="modal-select" onchange="cambiarCamposPorCultivo(this.value)">
      <option value="cafe" ${cultivoActual === 'cafe' ? 'selected' : ''}>Café</option>
      <option value="platano" ${cultivoActual === 'platano' ? 'selected' : ''}>Plátano</option>
      <option value="otros" ${cultivoActual === 'otros' ? 'selected' : ''}>Otros cultivos</option>
    </select>
    <div id="camposDinamicosLote"></div>
    <input id="areaLote" class="modal-input" type="number" step="0.01" placeholder="Área (ha)" value="${loteExistente?.area_ha || areaHa.toFixed(2)}">
  `;
  window.cambiarCamposPorCultivo = (cultivo) => {
    const container = document.getElementById('camposDinamicosLote');
    if (cultivo === 'cafe') {
      container.innerHTML = `
        <input id="variedad" class="modal-input" placeholder="Variedad" value="${loteExistente?.variedad || ''}">
        <input id="distancia" class="modal-input" placeholder="Distancia de siembra" value="${loteExistente?.distancia_siembra || ''}">
        <input id="fechaSiembra" class="modal-input" type="date" value="${loteExistente?.fecha_siembra || ''}">
        <select id="tipoLabor" class="modal-select">
          <option value="">Tipo de labor</option>
          <option ${loteExistente?.tipo_labor === 'zoca' ? 'selected' : ''}>zoca</option>
          <option ${loteExistente?.tipo_labor === 'nueva siembra' ? 'selected' : ''}>nueva siembra</option>
          <option ${loteExistente?.tipo_labor === 'renovacion' ? 'selected' : ''}>renovacion</option>
        </select>
        <select id="luminosidad" class="modal-select">
          <option value="">Luminosidad</option>
          <option ${loteExistente?.luminosidad === 'libre exposicion' ? 'selected' : ''}>libre exposicion</option>
          <option ${loteExistente?.luminosidad === 'sombra' ? 'selected' : ''}>sombra</option>
        </select>
      `;
    } else if (cultivo === 'platano') {
      container.innerHTML = `
        <input id="variedad" class="modal-input" placeholder="Variedad" value="${loteExistente?.variedad || ''}">
        <input id="distancia" class="modal-input" placeholder="Distancia de siembra" value="${loteExistente?.distancia_siembra || ''}">
        <input id="fechaSiembra" class="modal-input" type="date" value="${loteExistente?.fecha_siembra || ''}">
      ...
      container.innerHTML = `<input id="otrosAtributos" class="modal-input" placeholder="Observaciones" value="${loteExistente?.otros_atributos?.observaciones || ''}">`;
    }
  };
  cambiarCamposPorCultivo(cultivoActual);
  modal.style.display = 'flex';
  const guardar = async () => {
    const nombre = document.getElementById('loteNombre').value;
    const cultivo = document.getElementById('cultivoSelect').value;
    const area = parseFloat(document.getElementById('areaLote').value);
    if (!nombre || isNaN(area)) { toast('Complete nombre y área'); return; }
    let data = {
      finca_id: fincaSeleccionada.id,
      nombre, cultivo, area_ha: area,
      user_id: currentUser.id
    };
    if (cultivo === 'cafe' || cultivo === 'platano') {
      data.variedad = document.getElementById('variedad')?.value;
      data.distancia_siembra = document.getElementById('distancia')?.value;
      data.fecha_siembra = document.getElementById('fechaSiembra')?.value;
      data.tipo_labor = document.getElementById('tipoLabor')?.value;
      if (cultivo === 'cafe') data.luminosidad = document.getElementById('luminosidad')?.value;
    } else {
      data.otros_atributos = { observaciones: document.getElementById('otrosAtributos')?.value || '' };
    }
    if (geom) {
      const geomMulti = geom.type === 'Polygon' ? { type: 'MultiPolygon', coordinates: [geom.coordinates] } : geom;
      data.geom = geomMulti;
    }
    if (isEdit && loteExistente) {
      const { error } = await sb.from('lotes').update(data).eq('id', loteExistente.id);
      if (error) toast('Error: ' + error.message);
      else { toast('Lote actualizado'); if (layerTemp) layerTemp.remove(); }
    } else {
      const { error } = await sb.from('lotes').insert([data]);
      if (error) toast('Error: ' + error.message);
      else { toast('Lote guardado'); if (layerTemp) layerTemp.addTo(drawnItems); }
    }
    modal.style.display = 'none';
...
async function exportarMapaPDF() {
  ...
  try {
    ...
    container.style.width = '800px'; container.style.height = '600px'; container.style.backgroundColor = 'white';
    document.body.appendChild(container);
    const tempMapDiv = document.createElement('div'); tempMapDiv.style.width = '100%'; tempMapDiv.style.height = '500px';
    container.appendChild(tempMapDiv);
    const tempMap = L.map(tempMapDiv).setView([0,0], 10);
    L.tileLayer('https://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', { subdomains: ['mt0','mt1','mt2','mt3'] }).addTo(tempMap);
    if (fincaSeleccionada.geom) {
      let geom = fincaSeleccionada.geom;
      if (typeof geom === 'string') geom = JSON.parse(geom);
      L.geoJSON({ type: 'Feature', geometry: geom }, { style: { color: '#1565c0', weight: 3, fillOpacity: 0.1 } }).addTo(tempMap);
    }
    lotes.forEach(l => {
      if (!l.geom) return;
      let geom = l.geom; if (typeof geom === 'string') geom = JSON.parse(geom);
      L.geoJSON({ type: 'Feature', geometry: geom }, { style: { color: '#10b981', weight: 2, fillOpacity: 0.2 } }).addTo(tempMap);
    });
    if (lotes.length) {
      const bounds = L.geoJSON(lotes.filter(l => l.geom).map(l => { let g = l.geom; if (typeof g === 'string') g = JSON.parse(g); return { type: 'Feature', geometry: g }; })).getBounds();
      if (bounds.isValid()) tempMap.fitBounds(bounds, { padding: [40,40] });
    }
    await new Promise(r => setTimeout(r, 800));
    const mapCanvas = await html2canvas(tempMapDiv, { scale: 2, useCORS: true });
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: 'landscape', unit: 'mm', format: 'a4' });
    const imgWidth = 280; const imgHeight = (mapCanvas.height * imgWidth) / mapCanvas.width;
    pdf.addImage(mapCanvas.toDataURL('image/jpeg', 0.9), 'JPEG', 5, 10, imgWidth, imgHeight);
    pdf.setFontSize(14); pdf.text(`Finca: ${fincaSeleccionada.nombre}`, 5, imgHeight + 20);
    pdf.setFontSize(10);
    pdf.text(`Propietario: ${propietario?.nombre_completo || ''} - Cédula: ${propietario?.cedula || ''}`, 5, imgHeight + 30);
    pdf.text(`Área total: ${fincaSeleccionada.area_total_ha || '?'} ha`, 5, imgHeight + 38);
    pdf.text(`Fecha: ${new Date().toLocaleDateString('es-CO')}`, 5, imgHeight + 46);
    pdf.save(`Finca_${fincaSeleccionada.nombre}.pdf`);
    tempMap.remove(); container.remove();
    toast('PDF generado');
  } catch(err) { toast('Error: ' + err.message); }
}

// Event listeners (se asignan después de que el DOM esté listo)
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('editarPropietarioBtn').onclick = () => {
    ...
    document.getElementById('modalBody').innerHTML = `
      ...
      <input id="propCorreo" class="modal-input" placeholder="Correo electrónico" value="${propietario?.correo || (currentUser ? currentUser.email : '')}">
    `;
    modal.style.display = 'flex';
    const guardar = async () => {
      const data = {
  ...
  document.getElementById('nuevoLoteBtn').onclick = () => {
    if (!fincaSeleccionada) { toast('Selecciona una finca primero'); return; }
    iniciarDibujo('lote', (geom, areaHa, layer) => mostrarFormularioLote(null, geom, areaHa, layer));
  };
  document.getElementById('exportarMapaBtn').onclick = exportarMapaPDF;
});
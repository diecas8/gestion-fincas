...
function mostrarFormularioFinca(fincaExistente, geom, areaHa, layerTemp) {
  ...
  document.getElementById('modalConfirm').onclick = confirmar;
  document.getElementById('modalCancel').onclick = () => { modal.style.display = 'none'; if (layerTemp) layerTemp.remove(); };
}

// Formulario para lote con campos dinámicos según cultivo
function mostrarFormularioLote(loteExistente, geom, areaHa, layerTemp) {
  ...
  const confirmar = async () => {
    ...
    modal.style.display = 'none';
    if (fincaSeleccionada) cargarLotes(fincaSeleccionada.id).then(() => cargarCapasMapa());
  };
  document.getElementById('modalConfirm').onclick = confirmar;
  document.getElementById('modalCancel').onclick = () => { modal.style.display = 'none'; if (layerTemp) layerTemp.remove(); };
}

// Exportar a PDF (mapa de la finca con sus lotes)

// productor.js - lógica completa del productor (CORREGIDO Y COMPLETO)
...
function renderLotesList() {
  ...
  container.innerHTML = lotes.map(l => `
    <div class="lote-card" data-id="${l.id}">
      ...
      <span style="background:#e2e8f0; padding:2px 8px; border-radius:20px;">${l.cultivo}</span>
...
function iniciarDibujo(callback) {
  if (currentDrawControl) {
    try {
      ...
      console.warn('Error removiendo control:', e);
    }
...
function mostrarFormularioFinca(fincaExistente, geom, areaHa, layerTemp) {
  ...
  const guardar = async () => {
    ...
    console.log('Enviando datos de finca:', data);
...
// NUEVA FUNCIÓN PARA MOSTRAR FORMULARIO DE LOTES
function mostrarFormularioLote(geom, areaHa, layerTemp) {
  ...
  document.getElementById('modalBody').innerHTML = `
    ...
    <select id="loteCultivo" class="modal-select" style="width:100%;padding:12px;border-radius:12px;border:1px solid #e2e8f0;margin-bottom:12px;">
    ...
    <input id="loteVariedad" class="modal-input" placeholder="Variedad (ej: Castillo, Caturra)">
  ...
  const guardar = async () => {
    ...
    console.log('Enviando datos de lote:', data);
...
// CORREGIDO: Ahora sí guarda el lote
document.getElementById('nuevoLoteBtn').onclick = () => {
  if (!fincaSeleccionada) { 
    toast('Selecciona una finca primero'); 
    return; 
  }
